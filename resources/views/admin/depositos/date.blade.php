...
@section('content')
  ...
  <div class="card card-primary">
    <h2 class="text-center my-3">Alteração de data</h2>
      <hr>

    <h3 class="px-3">Depósito número: {{$deposito->numero}}</h3>
    <div class="card-body table-responsive p-4">
      <form action="{{route('depositos.putdate', ['id'=>$deposito->id])}}" method="POST">
        @csrf
        <div class="form-group">
          <label>Inserir data:</label>
          <div class="input-group">
            ...
            @else
            <input type="date" name="data" value="{{date('Y-m-d', strtotime($deposito->data))}}">
            @endif
          </div>
        </div>
